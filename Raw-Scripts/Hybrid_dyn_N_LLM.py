...
def n_nary_select_iterative_NSNode(classifier, query, all_segments, split_threshold = 0.1, attack_class=0,
    ...
    if split_threshold is None:
        split_threshold = max(1, int(0.1 * len(split_query)))  # Set split_threshold to 20% of the sentence length
    ...
    split_query = query.split() # Redundant line. Remove it later
...
def Hybrid_WNR(classifier, query, attack_class, n=2, k=7, split_threshold=None):
    """
    ...
    query_count = 0
    root_node = None
    initial_prob = classifier(query)[0][attack_class]["score"]  # Get the initial classification probability for the attack class
    query_count += 1  # Increment the count of classifier queries made
    ...
    while not done:
        ...
        if root_node:
            ...
            queries, root_node, segment_to_explore, all_segments= n_nary_select_iterative_NSNode(
                classifier, query, all_segments, split_threshold, attack_class,
                NSStruct=root_node, initial_prob=initial_prob, 
                n=n, previous_segment=segment_to_explore,
            )
        else:
            # Use n-ary search without a previously defined structure
         
            queries, root_node, segment_to_explore, all_segments = n_nary_select_iterative_NSNode(
                classifier, query, all_segments, split_threshold, attack_class,
                NSStruct=root_node, initial_prob=initial_prob, n=n
            )
...
def run_attack_experiments(
    ...
    with open(tsv_path, mode, encoding="utf-8", newline="") as f_out:
        ...
        for rel_idx in tqdm(
            ...
            n = get_n_gpt_neo(senlen, ds_name)
            print(n)
