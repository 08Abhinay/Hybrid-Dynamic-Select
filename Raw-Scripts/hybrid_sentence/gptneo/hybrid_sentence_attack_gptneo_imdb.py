...
# model_T5 = T5ForSequenceClassification.from_pretrained("/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/saved_models/T5-classsifier/T5-classifier-imdb/checkpoint-3125")
# tokenizer_T5 = AutoTokenizer.from_pretrained("/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/saved_models/T5-classsifier/T5-classifier-imdb/checkpoint-3125")
# # model = model.to(device)

#  # Load model & tokenizer from local checkpoint
# model_GPT2 = AutoModelForSequenceClassification.from_pretrained("/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/saved_models/GPT2_classifier/GPT2-classifier-imdb/checkpoint-6250")
# tokenizer_GPT2 = AutoTokenizer.from_pretrained("/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/saved_models/GPT2_classifier/GPT2-classifier-imdb/checkpoint-6250")
...
# -------------------------------------------------------------------
def run_attack_experiments(classifier, ds, k, n, split_threshold_percentage=0.1):
    """
    ...
    path = f"/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/Outputs_final/GPTneo/Hybrid_Sentence/{temp_ds_name}/k={k}/n={n}"
    out_filename = f"{temp_ds_name}_hybridsentence.tsv"
    ...
    with open(output_path, mode, encoding='utf-8', newline='') as hybrid_f:
        ...
        for idx in tqdm(range(start_idx, len(dataset)), desc="Running attack experiments"):
            ...
            if predicted_label != gold_label:
                ...
                # Attack
                success, final_query, query_count, final_prob = Hybrid_WNR(
                    classifier, text, gold_label, n, k, split_threshold_percentage
                )
...
# T5_predictor = SentimentPredictor(model_T5, tokenizer_T5, device)
# GPT2_predictor = SentimentPredictor(model_GPT2, tokenizer_GPT2, device)
   
imdb_deberta_classifier = pipeline("text-classification", 
                                   model="/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/saved_models/GPTneo_classifier/imdb/GPTneo_classifier_checkpoint", 
                                   max_length=512, truncation=True, return_all_scores=True, device=0, framework="pt") 
   
# Run experiments for each dataset, classifier, n, and k
def main():
    
    print("Starting attack for Hybrid Sentence for n=3, deberta")
    classifier = imdb_deberta_classifier
    run_attack_experiments(classifier, ds="imdb", k=-1, n=2, split_threshold_percentage=0.1)
    run_attack_experiments(classifier, ds="imdb", k=-1, n=3, split_threshold_percentage=0.1)
    run_attack_experiments(classifier, ds="imdb", k=-1, n=6, split_threshold_percentage=0.1)
    print("Finished!")
