...
def n_nary_select_iterative_NSNode(classifier, query, attack_class = 0, NSStruct = None, initial_prob = None, n = 2): #for analysis
    ...
    if(not NSStruct):
        if(not initial_prob):
            ...
            initial_prob = classifier.predict(query)[0][attack_class]["score"]
    ...
    while start < end:
      ...
      for i in range(len(queries)):
        cur_query = queries[i]
        cur_prob = classifier.predict(cur_query)[0][attack_class]["score"]
        query_count += 1
...
def wordnet_replace(classifier, query, initial_prob, replace_pos, attack_class):
    ...
    for cur_syn in syns:
        ...
        cur_preds = classifier.predict(cur_query)[0]
...
def run_attack_experiments(classifier, ds, k, n, greedy=False, replace="wordnet"):
    ...
    if greedy:
        path = f"/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/Outputs/T5/Greedy_revised/{temp_ds_name}/k={k}/n={n}"
        out_filename = f"{temp_ds_name}_Greedy_WNR.tsv"
    else:
        path = f"/scratch/gilbreth/abelde/NLP_Score_Based_Attacks/Outputs/T5/Nnary_revised/{temp_ds_name}/k={k}/n={n}"
...
def main():
    ...
    print("Starting attack for Nnary3 revised, T5")
    classifier = T5_predictor
    run_attack_experiments(classifier, ds="imdb", k=-1, n=3, greedy=False, replace="wordnet")
    print("Finished!")
...
def run_attack_experiments(
    ...
    """
    scheme = "Greedy" if greedy else "Nnary"
    ...
    with open(tsv_path, mode, encoding="utf-8", newline="") as f_out:
        ...
        for rel_idx in tqdm(
            ...
            # ---------------- predict ------------------------------------------
            orig_scores = classifier.predict(text)[0]
...
def main() -> None:
    ...
    for ds, m in zip(args.datasets, args.models):
        ...
        cls = SentimentPredictor(model, tokenizer, device)
